...
def lista_opcoes():
    ...
    if Opcao == 1:
        Calculo_de_Media()
    if Opcao == 2:
        Conversor_de_Metical()
    if Opcao == 3:
        tabuada()
    if Opcao == 4:
        Calcular_Quantidade_de_Tinta()
    if Opcao == 5:
        Contador_de_Letras()
    if Opcao == 6:
        print("Abrindo jogo da velha")
        Jogo_Da_Velha()


def Calculo_de_Media():
    global Calculo_de_Media

    #Calcule média de duas notas
    print('Seja bem vindo ao nosso Calcudador de Medias')
...
def Jogo_Da_Velha():
        ...
        class Tic_Tac_Toe():
            ...
            def display_gameover(self):
                ...
                self.canvas.create_text(size_of_board / 2, 15 * size_of_board / 16, font="cmr 20 bold", fill="blue",
                                        text=score_text)
